...
// It is assumed that no interfaces in the program have alternate settings.
// If not, please define a nonweakly function with the same name to override this function.
__WEAK bool USBD_User_GetInterface(uint8_t interface)
{
  ...
  for (i = 0; i <= 0xFF; i++)
  {
    cfg_desc = USBD_User_GetConfigDescriptor(i, &length);
    if (cfg_desc != NULL)
    {
      if (UsbdCoreInfo.Configuration == cfg_desc[5])    // bConfigurationValue
      {
        if (interface < cfg_desc[4])    // bNumInterfaces
        {
          EP0Buf = 0;
          UsbdCoreInfo.DataPtr = (uint8_t*)&EP0Buf;
          return true;
...
// It is assumed that no interfaces in the program have alternate settings.
// If not, please define a nonweakly function with the same name to override this function.
__WEAK bool USBD_User_SetInterface(uint8_t interface, uint8_t alternate_setting)
{
  const uint8_t* cfg_desc;
  uint16_t length;
  uint8_t i;
...
/**
 * @brief  Handle Get Interface request.
 * @return true - Success, false - Error
 */
static bool USBD_StdReqGetInterface(void)
{
  uint8_t interface;

  if (UsbdCoreInfo.DeviceState == USBD_STATE_CONFIGURED)
  {
    interface = UsbdCoreInfo.SetupPacket.wIndexL;
    if (USBD_User_GetInterface(interface)) {
      return true;
    }
  }
...
/**
 * @brief  Handle Set Interface request.
 * @return true - Success, false - Error
 */
static bool USBD_StdReqSetInterface(void)
{
  uint8_t interface;
  ...
  {
    interface = UsbdCoreInfo.SetupPacket.wIndexL;
    alternate_setting = UsbdCoreInfo.SetupPacket.wValueL;

    if (USBD_User_SetInterface(interface, alternate_setting)) {
      return true;
    }
  }
...
UsbdControlStage USBD_EndPoint0_Setup_StdReq(void)
{
  switch (UsbdCoreInfo.SetupPacket.bRequest) {
    ...
    case USB_REQUEST_GET_INTERFACE:
      if ((UsbdCoreInfo.SetupPacket.bmRequestType == 0x81) &&
          (UsbdCoreInfo.SetupPacket.wValue == 0) &&
          (UsbdCoreInfo.SetupPacket.wLength == 1))
      {
        if (USBD_StdReqGetInterface()) {
          return USBD_CONTROL_STAGE_IN_DATA;
        }
      }
      break;
    ...
    case USB_REQUEST_SET_INTERFACE:
      if ((UsbdCoreInfo.SetupPacket.bmRequestType == 0x01) &&
          (UsbdCoreInfo.SetupPacket.wLength == 0))
      {
        if (USBD_StdReqSetInterface()) {
          return USBD_CONTROL_STAGE_STATUS;
        }
      }
      break;
