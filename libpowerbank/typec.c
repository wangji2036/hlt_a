...
static void TC_Disable_Entry(struct tc_s * tc)
{
	...
	if(tc->tc_index == PORT1_INDEX) usb_dpdm_port1_switch(false);
...
static void TC_SNK_Attached_Entry(struct tc_s * tc)
{
    ...
    if(tc->tc_index == PORT1_INDEX && !gd->usb_comm_activated) usb_dpdm_port1_switch(true);
#else
...
static void TC_SNK_Attached_Exit(struct tc_s * tc)
{
    ...
    tc->cc1 = cc1;
    tc->cc2 = cc2;
	if(tc_snk_is_disconnected(tc)) //CC�Ͽ�
	{
		//tc->tc_timer_cnt++;
...
static void TC_SRC_Unattached_Entry(struct tc_s * tc)
{
	...
	/* USB_COM: redirect to SNK state machine for CC=Rd + Rp detection */
	if (gd->usb_comm_activated && tc->tc_index == 1) {
		usb_tc_set_state(tc, TC_SNK_Unattached, enter_state);
		return;
	}
#endif
...
static void TC_SRC_Attached_Exit(struct tc_s * tc)
{
	...
	if(tc_src_is_disconnected(tc)) //CC�Ͽ�
	{
		//tc->tc_timer_cnt++;
		if((uint32_t)(tc_sys_ticks - tc->tc_timer_cnt) > TC_T_PD_DEBOUNCE)
		{
