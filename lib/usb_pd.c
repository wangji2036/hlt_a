...
struct usb_pd_pkt_t g_pd_packet_1;
uint8_t g_pd_packet_cnt = 0;
...
void updata_pdo_of_source(const uint32_t * pdo,uint8_t n_pdo)
{
	...
	lib_printk("update source caps:%d\n",n_pdo);
}
...
static void PE_SNK_Wait_for_Capabilities_Exit(void)
{
	...
	{
		//lib_printk("SenderResponseTimer timeout\n");
...
{
	usb_pd_timer_stop(SenderResponseTimer);
...
uint32_t usb_pd_check_request(struct usb_pd_request_packet_t *rqt)
{
    ...
    lib_printk("rdo=0x%x\n",rqt->request.WORD);
    lib_printk("object_posiotion=%d pdo_n=%d\n",index,g_usb_pd_s.src_tx_pdo_n);
    ...
    switch (pdo->source_pdo[index - 1].BITS.FIX_BITS.fixed)
    {
        ...
        case PDO_TYPE_FIXED:
            ...
            break;
        case PDO_TYPE_APDO:
        	if(pdo->source_pdo[index - 1].BITS.PPS_BITS.apdo_type == APDO_TYPE_SPR_AVS)
        	{
        		uint16_t avs_max_volt = 20000;
				rdo_op_current = rqt->request.PPS_BITS.op_current * 50;
				voltage = rqt->request.PPS_BITS.output_voltage * 25;
				if(pdo->source_pdo[index - 1].BITS.SPR_AVS_BITS.i_15_20 == 0) avs_max_volt = 15000;
				if (voltage > avs_max_volt) return check_pps_voltage_error;
				if(voltage > 15000) pdo_max_current = pdo->source_pdo[index - 1].BITS.SPR_AVS_BITS.i_15_20 * 10;
				else pdo_max_current = pdo->source_pdo[index - 1].BITS.SPR_AVS_BITS.i_9_15 * 10;
...
static void PE_SRC_Negotiate_Capability_Exit(void)
{
	...
	lib_printk("request pdo =0x%x \n",g_pd_packet.msg.request.request.WORD);
	lib_printk("check_ret =%d \n",ret);
...
static void PE_SRC_Ready_Entry(void)
{
	...
	//osal_set_event(USB_TASK,TCPM_EVT_PD_READY);
	osal_start_timerEx(TCPM_PSREADY_TIMER, 500, 0, USB_TASK, TCPM_EVT_PD_READY);
	pdlib_tcpc_set_cc(g_tcpc.tc_port_map,TYPEC_CC_RP_3_0);
	//lib_printk("pps cnt =%d \n",usb_pd_timers[SourcePPSCommTimer].timer.time_cnt);
}
...
static void PE_DRS_Send_Swap_Entry(void)
{
	hal_tcpc_send_ctrl_mgs(PD_CTRL_DR_SWAP);
	g_usb_pd_s.pe_tran_cb_type = TRANSMITE_TYPE_PSREADY;
	usb_pd_set_state(PE_DRS_Send_Swap,exit_state);
	//lib_printk("pps cnt =%d \n",usb_pd_timers[SourcePPSCommTimer].timer.time_cnt);
	usb_pd_timer_start(SenderResponseTimer,tSenderResponseTime);
}

static void PE_DRS_Send_Swap_Exit(void)
{
	if(usb_pd_timer_is_timeout(SenderResponseTimer))
    {
...
{
	static bool packet_need_copy = false;
#if(CONFIG_USBPD_POWER_ROLR & USBPD_POWER_ROLR_SNK)
	if(usb_pd_event & USB_PD_EVT_SNK_ATTACHED)
	{
		usb_pd_set_state(PE_SNK_Startup,enter_state);
		lib_printk("usb_pd_EVT_SNK_ATTACHED\n");
		g_usb_pd_s.pe_prl_busy = 0;
		need_rechager = 0;
		usb_pd_event = 0;
	}
	...
	{
		usb_pd_set_state(PE_SNK_RSC_Disable,enter_state);
		lib_printk("usb_pd_EVT_SNK_UNATTACH\n");
		g_usb_pd_s.pe_prl_busy = 0;
		usb_pd_event = 0;
		need_rechager = 0;
	}
	...
	{
		usb_pd_set_state(PE_SRC_Startup,enter_state);
		lib_printk("usb_pd_EVT_SRC_ATTACHED\n");
		g_usb_pd_s.pe_prl_busy = 0;
		usb_pd_event = 0;
		need_rechager = 0;
	}
	...
	{
		usb_pd_set_state(PE_SNK_RSC_Disable,enter_state);
		lib_printk("usb_pd_EVT_SRC_UNATTACH\n");
		g_usb_pd_s.pe_prl_busy = 0;
		usb_pd_event = 0;
		need_rechager = 0;
	}
	...
	{
		lib_printk("usb_pd_EVT_RX_HARDRESET\n");
		g_usb_pd_s.pe_prl_busy = 0;
	...
	else if(usb_pd_event & USB_PD_EVT_RX_SOP_PACKET)
	{
		...
		lib_printk("packet cnt = %d\n",g_pd_packet_cnt);

		if(packet_need_copy)
		{
			g_pd_packet.hdr.WORD = g_pd_packet_1.hdr.WORD;
			g_pd_packet.msg_len = g_pd_packet_1.msg_len;
			g_pd_packet.sop_type = g_pd_packet_1.sop_type;
			for(uint8_t i = 0; i < 7; i++) g_pd_packet.msg.WORDS[i] = g_pd_packet_1.msg.WORDS[i];
			packet_need_copy = false;
			lib_printk("copy\n");
			g_pd_packet_cnt = 0;
		}

		lib_printk("header= 0x%x \n", g_pd_packet.hdr.WORD);
//		lib_printk("msg_type = %d \n ", g_pd_packet.hdr.BITS.message_type);
//
//		for(uint8_t i = 0; i < g_pd_packet.msg_len; i++)
//		{
//			lib_printk("0x%x ", pd_rx_buff[i]);
//		}
		lib_printk("\n");
		if(g_usb_pd_s.in_bist_mode == 1 || g_usb_pd_s.pe_prl_busy)
		{
			packet_need_copy = false;
			packet_need_copy = false;
			g_pd_packet_cnt = 0;
		...
		//lib_printk("msg_len = %d \n ", g_pd_packet.msg_len);
		usb_pd_sop_msg_handle();
		if(g_pd_packet_cnt) g_pd_packet_cnt--;

		if(g_pd_packet_cnt)
		{
			packet_need_copy = true;
			usb_pd_event |= USB_PD_EVT_RX_SOP_PACKET;
		}
	}
...
void usb_pd_run(void)
{
	...
	do
	{
		if(usb_pd_state_last != usb_pd_state || usb_pd_substate != usb_pd_substate_last) lib_printk("pe_state = %d %d\n", usb_pd_state,usb_pd_substate);
...
void __attribute__((isr)) USBPD_IRQHandler(void)
{
    ...
    do
    {
		...
		if(int_flag & (0x01<<10))
		{
			...
			if(g_pd_packet_cnt == 0)
			{
				g_pd_packet.hdr.WORD = TCPC->RXD_INFO.WORD;
				for(uint8_t i = 0; i < 7; i++) g_pd_packet.msg.WORDS[i] = pd_rx_buff[i];
				g_pd_packet.msg_len = rx_cnt;
				g_pd_packet.sop_type = TCPC->RXD_INFO.BITS.RXD_SOP_TYP;
			}
			else
			{
				g_pd_packet_1.hdr.WORD = TCPC->RXD_INFO.WORD;
				for(uint8_t i = 0; i < 7; i++) g_pd_packet_1.msg.WORDS[i] = pd_rx_buff[i];
				g_pd_packet_1.msg_len = rx_cnt;
				g_pd_packet_1.sop_type = TCPC->RXD_INFO.BITS.RXD_SOP_TYP;
			}
			g_pd_packet_cnt++;
			rx_cnt = 0;
