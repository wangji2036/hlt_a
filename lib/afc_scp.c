...
{
	if(lib_para.scp_source_support == 0)
	{
		if(scp_packet.bytes.msg_1 >= SCP_REG_ADP_TYPE0)
		{
			DPDM->AFC_TX_0.WORD =((FCP_NACK << 8) | 0x01);
			osal_set_event(USB_DPDM_TASK,DPDM_EVT_SCP_TX_DATA);
			scp_tx.words[0] = ((FCP_NACK << 8) | 0x01);
			scp_tx.words[1] = 00;
			scp_tx.words[2] = 00;
		}
		else
...
if(lib_para.afc_source_support)
{
	scp_iout = 3300;
