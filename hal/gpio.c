...
void hal_gpio_init(void)
{
	...
	else
	{
		...
		GPA->PUEN.BITS.PIN5 = 0;
		GPA->PDEN.BITS.PIN5 = 0;
		GPA->MODE.BITS.PIN5 = 2; //00:PA5 01:SCL3_S 10:LS_ISNS_PGA_N 11:RESERVED
	}
	...
	else
	{
		...
		GPB->PUEN.BITS.PIN5 = 0;
		GPB->PDEN.BITS.PIN5 = 0;
		GPB->MODE.BITS.PIN5 = 3; //00:PB5 01:JTAG_RST 10:BADC6 11:LS_ISNS_PGA_P
	}
...
{
	/* PA0 */
	GPA->I_EN.BITS.PIN0 = 0;//1;
	GPA->O_EN.BITS.PIN0 = 0;//0;
	GPA->DOUT.BITS.PIN0 = 0;//0;
	GPA->ODEN.BITS.PIN0 = 0;//1;
	GPA->PUEN.BITS.PIN0 = 0;//0;
	GPA->PDEN.BITS.PIN0 = 0;//0;
	GPA->MODE.BITS.PIN0 = 0;//0; //00:SCL1_S 01:PA0 10:UART2_TXD 11:DP_C

	/* PA1 */
	GPA->I_EN.BITS.PIN1 = 0;//1;
	GPA->O_EN.BITS.PIN1 = 0;//0;
	GPA->DOUT.BITS.PIN1 = 0;//0;
	GPA->ODEN.BITS.PIN1 = 0;//1;
	GPA->PUEN.BITS.PIN1 = 0;//0;
	GPA->PDEN.BITS.PIN1 = 0;//0;
	GPA->MODE.BITS.PIN1 = 0;//0; //00:SDA1_S 01:PA1 10:UART2_RXD 11:DM_C
	GPA->ITEN.BITS.PIN1 = 0;//0;
	GPA->ITTP.BITS.PIN1 = 0;//0; //00:Falling Edge 01:Rising Edge 1x:both edge
	...
	{
		GPA->I_EN.BITS.PIN4 = 0;//1;
		GPA->O_EN.BITS.PIN4 = 0;//0;
		GPA->DOUT.BITS.PIN4 = 0;
		GPA->ODEN.BITS.PIN4 = 0;
		GPA->PUEN.BITS.PIN4 = 0;
		GPA->PDEN.BITS.PIN4 = 0;
	...
	GPA->O_EN.BITS.PIN6 = 0;//1;
	GPA->DOUT.BITS.PIN6 = 0;//1;
	GPA->ODEN.BITS.PIN6 = 0;//0;
	GPA->PUEN.BITS.PIN6 = 0;//0;
	GPA->PDEN.BITS.PIN6 = 0;
	GPA->MODE.BITS.PIN6 = 0; //00:PA6 01:SCL2_M 10:RESERVED 11:RESERVED

	/* PA7 */
	GPA->I_EN.BITS.PIN7 = 0;
	GPA->O_EN.BITS.PIN7 = 0;//1;
	GPA->DOUT.BITS.PIN7 = 0;//1;
	GPA->ODEN.BITS.PIN7 = 0;//0;
	GPA->PUEN.BITS.PIN7 = 0;
	GPA->PDEN.BITS.PIN7 = 0;
	GPA->MODE.BITS.PIN7 = 0; //00:PA7 01:SDA2_M 10:RESERVED 11:RESERVED
	...
	GPB->ODEN.BITS.PIN0 = 0;
	GPB->PUEN.BITS.PIN0 = 0;
	GPB->PDEN.BITS.PIN0 = 0;//0;
	GPB->MODE.BITS.PIN0 = 0;//1; //00:PB0 01:OSC_IN 10:RESERVED 11:RESERVED

	/* PB1 */
	GPB->I_EN.BITS.PIN1 = 0;//0;
	GPB->O_EN.BITS.PIN1 = 0;
	GPB->DOUT.BITS.PIN1 = 0;
	GPB->ODEN.BITS.PIN1 = 0;
	GPB->PUEN.BITS.PIN1 = 0;
	GPB->PDEN.BITS.PIN1 = 0;//0;
	GPB->MODE.BITS.PIN1 = 0;//1; //00:PB1 01:OSC_OUT 10:RESERVED 11:RESERVED

	/* PB2 */
	if (SYS->PID_INFO.BITS.PID == NU17111)
	{
		GPB->I_EN.BITS.PIN2 = 0;//0;
		GPB->O_EN.BITS.PIN2 = 0;//1;
		GPB->DOUT.BITS.PIN2 = 0;//1;
		GPB->ODEN.BITS.PIN2 = 0;//0;
		GPB->PUEN.BITS.PIN2 = 0;//0;
		GPB->PDEN.BITS.PIN2 = 0;//0;
		GPB->MODE.BITS.PIN2 = 0;//1; //00:PB2 01:BPWM3 10:BADC2 11:DP_C2
	}
	else
	{
		GPB->I_EN.BITS.PIN2 = 0;//1;
		GPB->O_EN.BITS.PIN2 = 0;
		GPB->DOUT.BITS.PIN2 = 0;
		GPB->ODEN.BITS.PIN2 = 0;
		GPB->PUEN.BITS.PIN2 = 0;
		GPB->PDEN.BITS.PIN2 = 0;
		GPB->MODE.BITS.PIN2 = 0; //00:PB2 01:BPWM3 10:BADC2 11:DP_C2
	}
	...
	if (SYS->PID_INFO.BITS.PID == NU17111)
	{
		GPB->I_EN.BITS.PIN5 = 0;//1;
		GPB->O_EN.BITS.PIN5 = 0;
		GPB->DOUT.BITS.PIN5 = 0;
		GPB->ODEN.BITS.PIN5 = 0;//0;
		GPB->PUEN.BITS.PIN5 = 0;
		GPB->PDEN.BITS.PIN5 = 0;
	...
	{
		GPB->I_EN.BITS.PIN5 = 0;//0;
		GPB->O_EN.BITS.PIN5 = 0;//1;
		GPB->DOUT.BITS.PIN5 = 0;
		GPB->ODEN.BITS.PIN5 = 0;
		GPB->PUEN.BITS.PIN5 = 0;
		GPB->PDEN.BITS.PIN5 = 0;
	...
	/* PB7 */
	GPB->I_EN.BITS.PIN7 = 0;
	GPB->O_EN.BITS.PIN7 = 0;//1;
	GPB->DOUT.BITS.PIN7 = 0;//1;
	GPB->ODEN.BITS.PIN7 = 0;//0;
	GPB->PUEN.BITS.PIN7 = 0;//1;
	GPB->PDEN.BITS.PIN7 = 0;//0;
	GPB->MODE.BITS.PIN7 = 0;//1; //00:PB7 01:UART1_TXD 10:RESERVED 11:RESERVED


	/* PC0 */
	GPC->I_EN.BITS.PIN0 = 0;
	GPC->O_EN.BITS.PIN0 = 0;//1;
	GPC->DOUT.BITS.PIN0 = 0;//0;
	GPC->ODEN.BITS.PIN0 = 0;//0;
	GPC->PUEN.BITS.PIN0 = 0;//0;
	GPC->PDEN.BITS.PIN0 = 0;//0;
	GPC->MODE.BITS.PIN0 = 0;//1; //00:PC0 01:EPWM1 10:RESERVED 11:RESERVED

	/* PC1 */
	GPC->I_EN.BITS.PIN1 = 0;//0;
	GPC->O_EN.BITS.PIN1 = 0;//1;
	GPC->DOUT.BITS.PIN1 = 0;
	GPC->ODEN.BITS.PIN1 = 0;
	GPC->PUEN.BITS.PIN1 = 0;
	GPC->PDEN.BITS.PIN1 = 0;
	GPC->MODE.BITS.PIN1 = 0;//1; //00:PC1 01:EPWM2 10:RESERVED 11:RESERVED

	/* PC2 */
	GPC->I_EN.BITS.PIN2 = 0;//1;
	GPC->O_EN.BITS.PIN2 = 0;
	GPC->DOUT.BITS.PIN2 = 0;
	GPC->ODEN.BITS.PIN2 = 0;
	GPC->PUEN.BITS.PIN2 = 0;
	...
	/* PC6 */
	GPC->I_EN.BITS.PIN6 = 0;//1;
	GPC->O_EN.BITS.PIN6 = 0;
	GPC->DOUT.BITS.PIN6 = 0;
	GPC->ODEN.BITS.PIN6 = 0;
	GPC->PUEN.BITS.PIN6 = 0;//1;
	GPC->PDEN.BITS.PIN6 = 0;
	GPC->MODE.BITS.PIN6 = 0; //00:PC6 01:BADC1 10:ECAP4 11:RESERVED
	GPC->ITEN.BITS.PIN6 = 0;
	GPC->ITTP.BITS.PIN6 = 0; //00:Falling Edge 01:Rising Edge 1x:both edge
	...
	GPD->MODE.BITS.PIN3 = 0;//1; //00:PD3 01:BADC9 10:RESERVED 11:RESERVED
	GPD->ITEN.BITS.PIN3 = 0;
	GPD->ITTP.BITS.PIN3 = 0; //00:Falling Edge 01:Rising Edge 1x:both edge

	/* PD4 */
	GPD->I_EN.BITS.PIN4 = 0;//1;
	GPD->MODE.BITS.PIN4 = 0;//1; //00:PD4 01:ECAP1 10:RESERVED 11:RESERVED

	/* PD5 */
	GPD->I_EN.BITS.PIN5 = 0;//1;
	GPD->MODE.BITS.PIN5 = 0;//1; //00:PD5 01:ECAP2 10:RESERVED 11:RESERVED

	/* PD6 */
	GPD->I_EN.BITS.PIN6 = 0;//1;
	GPD->MODE.BITS.PIN6 = 0;//1; //00:PD6 01:BADC3 10:RESERVED 11:RESERVED
}
