...
void SLP_vNormalToSleep(void)
{
	...
	if (!(gd->tc0_lighting_mode || gd->bat_dead_flag_with_snk0))
		pdlib_tcpc_set_cc(TYPEC_PORT_A, TYPEC_CC_OPEN);
	else
		pdlib_tcpc_set_cc(TYPEC_PORT_A, TYPEC_CC_RP_DEF);
#endif
#if (CONFIG_TYPECB_SUPPORT == 1)
	if (!(gd->tc1_lighting_mode || gd->bat_dead_flag_with_snk1))
		pdlib_tcpc_set_cc(TYPEC_PORT_B, TYPEC_CC_OPEN);
	else
		pdlib_tcpc_set_cc(TYPEC_PORT_B, TYPEC_CC_RP_DEF);
#endif
