...
uint8_t qfod_detect(void)
{
	...
	wpc_printk(" idle: [%d] [q:%d,%d,%d,%d] [f:%d,%d,%d,%d] [t:%d,%d] [fo:%d,%d]\n",
...
void wpc_idle_phase_process(void)
{
	...
	if (gd->bat_dead_flag)
		return;
	if (gd->adp.adp_type == EADP_TYPE_POWERBANK_WIRELESS_ONLY && (gd->ptx_idle_phase_status == WPC_IDLE_STAT_STANDBY || gd->ptx_idle_phase_status == WPC_IDLE_STAT_XER_FOD || gd->ptx_idle_phase_status == WPC_IDLE_STAT_QDT_FOD || gd->ptx_idle_phase_status == WPC_IDLE_STAT_LAR_MET))
	{
	    ...
	    gd->prot_sts.vpwr_ovp_flag || gd->prot_sts.pout_opp_flag || gd->bat_ov_forbid_flag)
	{
		...
		if (gd->prot_sts.tntc_otp_flag)
			wpc_printk("[tntc_otp:%d]", gd->sys_infos.ntc_temp_wpc);
		if (gd->prot_sts.tntc_utp_flag)
			wpc_printk("[tntc_utp:%d]", gd->sys_infos.ntc_temp_wpc);
		if (gd->prot_sts.tdie_otp_flag)
			wpc_printk("[tdie_otp:%d]", gd->sys_infos.die_temp);
		if (gd->prot_sts.tdie_utp_flag)
			wpc_printk("[tdie_utp:%d]", gd->sys_infos.die_temp);
		if (gd->prot_sts.isns_ocp_flag)
			wpc_printk("[isns_ocp:%d]", gd->isns);
		if (gd->prot_sts.vbus_ovp_flag)
			wpc_printk("[vbus_ovp:%d]", gd->vbus);
		if (gd->prot_sts.vbus_uvp_flag)
			wpc_printk("[vbus_uvp:%d]", gd->vbus);
		if (gd->prot_sts.vbus_dpl_flag)
			wpc_printk("[vbus_dpl:%d]", gd->vbus);
		if (gd->prot_sts.vpwr_ovp_flag)
			wpc_printk("[vpwr_ovp:%d]", gd->vpwr);
		if (gd->prot_sts.pout_opp_flag)
			wpc_printk("[pout_opp:%d %d]", gd->vpwr, gd->isns);
		if (gd->bat_ov_forbid_flag)
			wpc_printk("[bat_ov_forbid]");
		return;
	}
