...
static void ui_update_led(void)
{
	...
	else if (g_port.is_mini_current_mode)
	{
		...
		else if (gd->vpwr > 6200 && g_port.port_state[PORT0_INDEX] != PORT_STATE_NONE && !buckboost_protection_flag && (g_port.port_state[PORT0_INDEX] == PORT_STATE_SOURCE || gd->real_soc_show < 100))
		{
...
*/
	...
	if (soc_show_ram_led & BIT(0))
		drv_IO_control(disp_map[0], false);
	else
		drv_IO_control(disp_map[0], true);

	if (soc_show_ram_led & BIT(1))
		drv_IO_control(disp_map[1], false);
	else
		drv_IO_control(disp_map[1], true);

	if (soc_show_ram_led & BIT(2))
		drv_IO_control(disp_map[2], false);
	else
		drv_IO_control(disp_map[2], true);

	if (soc_show_ram_led & BIT(3))
		drv_IO_control(disp_map[3], false);
	else
		drv_IO_control(disp_map[3], true);

	if (soc_show_ram_led & BIT(4))
		drv_IO_control(disp_map[4], false);
	else
		drv_IO_control(disp_map[4], true);

	if (soc_show_ram_led & BIT(5))
		drv_IO_control(disp_map[5], false);
	else
		drv_IO_control(disp_map[5], true);
...
void key_handle_10ms()
{
	...
	{
		/* 短按一次后，再按住 8s 触发船运模式。 */
		if (!ship_key_triggered && key_cnt < 1800)
			key_cnt++;
		if (key_cnt == 1)
		{
