...
static void usb_bridge_prod_window_write(uint8_t offset, const uint8_t *data, uint8_t len)
{
	...
	if (len > PROD_WINDOW_DATA_SIZE)
		len = PROD_WINDOW_DATA_SIZE;
	window_buf[0] = offset;
	window_buf[1] = len;
...
void usb_bridge_periodic_update(void)
{
	...
	else if (cnt == 10)
	{
		...
		if (gd->eng_mode_active)
		{
			if (gd->eng_virtual_cell1 != VIRTUAL_CELL_SENTINEL)
				c1 = gd->eng_virtual_cell1;
			if (gd->eng_virtual_cell2 != VIRTUAL_CELL_SENTINEL)
				c2 = gd->eng_virtual_cell2;
		}
	...
	else if (cnt == 12)
	{
		...
		if (exc_cursor_page >= LOG_PAGE_COUNT)
			exc_cursor_page = 0;
		...
		{
			if (exc_page_counts[exc_cursor_page] > 0)
				break;
		...
		if (battery_record_read_by_page_index(exc_cursor_page, exc_cursor_idx, &rec) && rec.record_id != 0)
		{
