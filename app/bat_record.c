...
#define BR_OVER_VOLTAGE_THRESHOLD 4530                   //4400                         // 4.51V record threshold (mV)
...
static void write_exception_record(BatteryExceptionRecord_t *record)
{
	...
	// Check if Flash exception records are full → trigger OV_FORBID
	{
		...
		if (flash_total >= MAX_TOTAL_RECORDS)
		{
			...
			xgb_printk("[BR] EXCEPTION FULL (%d/%d) -> FORBID!\n",
...
static void migrate_old_format(void)
{
	...
	uint8_t records_to_copy = (old_storage.exception_counter < 5) ? old_storage.exception_counter : 5;
	...
	for (uint8_t i = 1; i < LOG_PAGE_COUNT; i++)
	{ // Start from 1, skip LOG1
		memset(&g_flash_page_buffer, 0, sizeof(FlashPageLayout_t));
		g_flash_page_buffer.magic = MAGIC_VALUE;
		g_flash_page_buffer.page_records_count = 0;
		g_flash_page_buffer.page_number = i;
...
void battery_record_init(void)
{
	...
	else if (has_new_format)
	{
        ...
        if (!verify_storage_checksum(&g_record_storage)) {
            ...
            need_init = true;
            xgb_printk("[BR-INIT] CHECKSUM FAIL! page=%d wptr=%d cnt=%d\n",
                   g_record_storage.active_page, g_record_storage.write_ptr,
                   g_record_storage.exception_counter);
        } else {
	    ...
	    g_exception_cache.temp_dchg_triggered > 1)
	{
		...
		xgb_printk("[BR-INIT] cache cleared (invalid data)\n");
	}
	else
	{
		xgb_printk("[BR-INIT] cache preserved (window continues)\n");
	}
}
...
{
	if (cell_voltage < BR_OVER_VOLTAGE_THRESHOLD)
		return;
...
void battery_record_update_overvoltage(void)
{
	...
	xgb_printk("[BR] OV: c1=%d c2=%d thr=%d eng=%d vc1=%d vc2=%d\n",
	...
	xgb_printk("[BR] OV: cell1=%dmV thr=%d eng=%d virt_cell1=%d\n",
...
#define ENG_CHRG_OT_TEMP_DEGC 550 //600   /* 60.0°C charging record threshold (0.1°C) */
#define ENG_DISG_OT_TEMP_DEGC 600 //650   /* 65.0°C discharging record threshold (0.1°C) */
...
static void process_window_end(void)
{
	...
	g_exception_cache.ov1_triggered = 0;
	g_exception_cache.ov1_max_voltage = 0;
	g_exception_cache.ov2_triggered = 0;
	g_exception_cache.ov2_max_voltage = 0;
	g_exception_cache.temp_chg_triggered = 0;
	g_exception_cache.temp_chg_max = 0;
	g_exception_cache.temp_dchg_triggered = 0;
	g_exception_cache.temp_dchg_max = 0;
}
...
void battery_record_periodic_check(void)
{
	...
	if (++check_counter >= 10)
	{
		    ...
		    g_exception_cache.window_start_seconds == 0xFFFFFFFF)
		{
			...
			VIC_vModuleEnable();
			xgb_printk("[BR] Window initialized\n");
		}

		xgb_printk("[BR] periodic: eng=%d mode=%d cell1=%d temp=%d\n",
		...
		if (gd->eng_mode_active && g_eng_virtual_triggered)
		{
			...
			{
				process_window_end();
				xgb_printk("[BR] Eng mode: immediate flush\n");
			}
		}
		...
		if (is_new_hour(g_exception_cache.window_start_seconds, current_seconds))
		{
			...
			xgb_printk("[BR] New window started\n");
		}

		TimeStamp_t ts;
		get_current_timestamp(&ts);
		uint32_t elapsed = current_seconds - g_exception_cache.window_start_seconds;
		xgb_printk("RTC: %04d-%02d-%02d %02d:%02d:%02d W:%u/%ds\n",
...
{
	if (!read_record_from_flash(page, index, record))
		return false;
	return true;
}
...
void battery_record_print_next_log(void)
{
	...
	for (uint8_t i = 0; i < LOG_PAGE_COUNT; i++)
	{
		...
		{
			page_counts[i] = (page_headers[i].page_records_count <= MAX_RECORDS_PER_PAGE) ? page_headers[i].page_records_count : MAX_RECORDS_PER_PAGE;
	...
	if (page_counts[active] > 0)
	{
		...
		for (int16_t i = page_counts[active] - 1; i >= 0; i--)
		{
			...
			case EXCEPTION_TYPE_OVERVOLTAGE:
				xgb_printk("OV Cell%d: %dmV (Total:%dmV)",
				           record.sub_type,
			...
			default:
				xgb_printk("Unknown type: 0x%02X", record.error_type);
				break;
			}
	...
	{
		if (p == active)
			continue; // Skip active page (already printed)
		if (page_counts[p] == 0)
...
uint8_t battery_record_sleep_check(void)
{
	...
	xgb_printk("[BR-SLEEP] page=%d wptr=%d cnt=%d win=%u ov1=%d\n",
	           g_record_storage.active_page, g_record_storage.write_ptr,
	           g_record_storage.exception_counter,
	...
	GPB->MODE.BITS.PIN6 = 1; /* PB6 → BADC7 (BAT2+) */
	GPC->I_EN.BITS.PIN7 = 1;
	GPC->MODE.BITS.PIN7 = 1; /* PC7 → BADC4 (Cell2) */
	GPD->I_EN.BITS.PIN3 = 1;
	GPD->MODE.BITS.PIN3 = 1; /* PD3 → BADC9 (VBAT-) */
	...
	GPB->MODE.BITS.PIN6 = 0;
	GPB->I_EN.BITS.PIN6 = 0; /* PB6 → GPIO */
	GPC->MODE.BITS.PIN7 = 0;
	GPC->I_EN.BITS.PIN7 = 0; /* PC7 → GPIO */
	GPD->MODE.BITS.PIN3 = 0;
	GPD->I_EN.BITS.PIN3 = 0; /* PD3 → GPIO */
	...
	{
		uint8_t mode = g_buckboost.woke_mode;
		if (mode == BUCKBOOST_SHUTDOWM_MODE)
			mode = BUCKBOOST_DISCHG_MODE;
		uint8_t event_type;
	...
	uint32_t elapsed = current_seconds - g_exception_cache.window_start_seconds;
	xgb_printk(" W:%u/%ds", elapsed, EXCEPTION_WINDOW_SECONDS);
	if (is_new_hour(g_exception_cache.window_start_seconds, current_seconds))
	{
		if (g_exception_cache.ov1_triggered || g_exception_cache.ov2_triggered ||
		    g_exception_cache.temp_chg_triggered || g_exception_cache.temp_dchg_triggered)
...
uint16_t battery_record_get_overtemp_count(void)
{
	...
	for (uint8_t page = 0; page < LOG_PAGE_COUNT; page++)
	{
		...
		if (g_flash_page_buffer.magic == MAGIC_VALUE)
		{
			uint8_t records_in_page = (g_flash_page_buffer.page_records_count <= MAX_RECORDS_PER_PAGE) ? g_flash_page_buffer.page_records_count : MAX_RECORDS_PER_PAGE;
