...
void mpp_report_pkt_process(struct mpp_prx_ask_pkt_t *mpp_ask)
{
	...
	if (mpp_ask->msg.report_pla.select == 1)//TODO: add fod func here, add a timer print log to avoid the FSK window
	{
		...
		if (res == 0)
		{
			...
			else if((wirless_ntc_power_reduce || wpc_ntc_power_reduce_flag || gd->bat_ntc_dischg_reduce_flag || bat_ntc_dischg_ut_reduce_flag) && renego_flag == 1)
			{
				fml_fsk_patt_send(EPWM1, T_RESPONSE, _FSK_ATN);
				renego_flag = 2;
			}
			else if(!wirless_ntc_power_reduce && !wpc_ntc_power_reduce_flag && !gd->bat_ntc_dischg_reduce_flag && !bat_ntc_dischg_ut_reduce_flag && renego_flag == 3)
			{
				fml_fsk_patt_send(EPWM1, T_RESPONSE, _FSK_ATN);
				renego_flag = 4;
			}
...
void wpc_mpp_xfer_phase_protocol_process(struct com_prx_ask_pkt_t *com_ask)
{
	...
	switch (mpp_ask->hdr)
	{
		...
		case MPP_PRx_PKT_TYP_XCE_19:
			...
			{
				if(wirless_ntc_power_reduce || wpc_ntc_power_reduce_flag || gd->bat_ntc_dischg_reduce_flag || bat_ntc_dischg_ut_reduce_flag)
				{
					if (gd->tx_power > 8500)
					{
						gd->rx_infos.cep_val = -4;
			...
			if((wirless_ntc_power_reduce || wpc_ntc_power_reduce_flag || gd->bat_ntc_dischg_reduce_flag || bat_ntc_dischg_ut_reduce_flag) && renego_flag == 0)
			{
				gd->tx_infos.nego_cap = 75;
				renego_flag = 1;
			}
			else if(!wirless_ntc_power_reduce && !wpc_ntc_power_reduce_flag && !gd->bat_ntc_dischg_reduce_flag && !bat_ntc_dischg_ut_reduce_flag && renego_flag == 2)
			{
				renego_flag = 3;
