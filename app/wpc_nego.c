...
uint8_t power_contract_change_cnt(struct power_transfer_contract_t *ptx_contract, struct power_transfer_contract_t *prx_contract)
{
	...
	if (prx_contract->rep_delay != ptx_contract->rep_delay)
		change_cnt++;

	if (prx_contract->pch_delay != ptx_contract->pch_delay)
		change_cnt++;

	if (prx_contract->freq_sel != ptx_contract->freq_sel)
		change_cnt++;

	if (prx_contract->ext_nego_power != ptx_contract->ext_nego_power)
		change_cnt++;

	if (prx_contract->power_control_profile != ptx_contract->power_control_profile)
		change_cnt++;

	if (prx_contract->cloak_det_ping_delay != ptx_contract->cloak_det_ping_delay)
		change_cnt++;

	if (prx_contract->cloak_dig_ping_delay != ptx_contract->cloak_dig_ping_delay)
		change_cnt++;
