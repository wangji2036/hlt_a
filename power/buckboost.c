...
#define NU6801_VBUS_OVP_TH 20000
	...
	{
		if (g_port.port_state[0] != PORT_STATE_SOURCE && g_port.port_state[3] != PORT_STATE_SOURCE)
			status &= ~VBUS_FUALT_VBAT_UVP;
	...
	if (status != 0 || gd->bat_ntc_dischg_lock == 3)
	{
		...
		if (status & (VBUS_FUALT_VBUS_SCP | VBUS_FUALT_VBUS_OVP | VBUS_FUALT_VBUS_OCP | VBUS_FUALT_VBAT_UVP | VBUS_SOFT_PROTECT | NTC_PCT | VBUS_FAULT_VBUS_NTC | NTC_SWITCH) || gd->bat_ntc_dischg_lock == 3)
		{
			...
			buckboost_ops.init();
			if (status & VBUS_FAULT_VBUS_NTC)
				hal_nu6805_buckboost_set_ovp(20000);
...
#define MEDIAN3(a, b, c) ((a) > (b) ? ((b) > (c) ? (b) : ((a) > (c) ? (c) : (a))) \
	...
	case BUCKBOOST_EVT_DUMMYLOADOVER:
		...
		if (g_buckboost.woke_mode == BUCKBOOST_DISCHG_MODE)
			buckboost_ops.set_ovp(g_buckboost.buckboost_out_voltage);
#endif
