...
> **v1.4 变更**: 第三节新增 §3.1 工程测试寄存器（i2c_buff 0x82~0x89，虚拟覆写 + 记录清除）；
> 更新版本历史。
>
...
### 3.1 工程测试寄存器 (0x82~0x89, 虚拟覆写 + 记录清除)

| 寄存器地址 | 名称 | 类型 | 方向 | HID CMD | NU17112 处理 | 说明 |
|-----------|------|------|------|---------|-------------|------|
| 0x82~0x83 | REG_ENG_VIRTUAL_CELL1 | u16 LE | PC→WB7720→NU17112 | CMD_WRITE_REGISTER (0x0C) | 替代真实 Cell1 电压采样 | 虚拟电芯1电压 (mV), 0=用真实值 |
| 0x84~0x85 | REG_ENG_VIRTUAL_CELL2 | u16 LE | PC→WB7720→NU17112 | 同上 | 替代真实 Cell2 电压采样 | 虚拟电芯2电压 (mV), 0=用真实值 |
| 0x86~0x87 | REG_ENG_VIRTUAL_TEMP | s16 LE | PC→WB7720→NU17112 | 同上 | 替代真实温度采样 | 虚拟温度 (0.1°C), 0=用真实值 |
| 0x88 | REG_ENG_ERASE_ALL_CMD | u8 | PC→WB7720→NU17112 | 同上 | 0xEE → 擦除全部异常记录 | 写 0xEE 触发清除 |
| 0x89 | REG_ENG_CMD_STATUS | u8 | NU17112→WB7720→PC | — (只读轮询) | 写入执行状态 | 0x00=空闲, 0x01=进行中, 0x02=成功, 0xFF=失败 |

**写保护**: 0x82~0x88 写入要求 `WORK_MODE(0x50)=0xA5`。

**虚拟覆写触发阈值**:
- 单节电压 >4450mV → OV (过压) 异常
- 温度 >600 (60.0°C) → OT (过温) 异常
...
| **v1.4** | **2026-02-28** | **工程测试寄存器**: 第三节新增 §3.1 工程测试寄存器（i2c_buff 0x82~0x89, 8B）；支持虚拟电芯电压(Cell1/Cell2)/温度覆写用于异常触发测试；支持 0xEE 命令清除全部异常记录 + CMD_STATUS 状态反馈；写保护要求 WORK_MODE=0xA5 |
