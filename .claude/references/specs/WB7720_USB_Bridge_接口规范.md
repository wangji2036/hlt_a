...
| CMD | 名称 | Payload | 说明 |
|-----|------|---------|------|
| `0x01` | CMD_READ_STATUS | 无 | 请求遥测或异常日志 |
| `0x02` | CMD_READ_DEVICE_INFO | 无 | 请求设备信息 |
| `0x0C` | CMD_WRITE_REGISTER | `[REG_ADDR][DATA_LEN][DATA...]` | 写 i2c_buff 寄存器 |
| `0x10` | CMD_FAST_CHARGE_ACTIVATE | 无 | 激活快充 |
...
### 3.4 生产模式寄存器 (上位机写入 ProductInfo)

| 地址 | 宏名 | 长度 | 方向 | NU17112 目标 |
|------|------|------|------|-------------|
| 0x90 | PROD_MODE_FLAG | 1B | PC→WB7720 | NU17112 轮询: 0xB5=进入生产模式 |
| 0x91 | PROD_WRITE_STATUS | 1B | NU17112→WB7720→PC | 0x01=进行中, 0x02=成功, 0xFF=失败 |
| 0x92~0xA5 | PROD_MANUFACTURER | 20B | PC→WB7720→NU17112 | Flash `ProductInfo_t.manufacturer_name` |
| 0xA6~0xB9 | PROD_MODEL | 20B | PC→WB7720→NU17112 | Flash `ProductInfo_t.model_name` |
| 0xBA~0xCD | PROD_BATTERY_MFR | 20B | PC→WB7720→NU17112 | Flash `ProductInfo_t.battery_mfr` |
| 0xCE~0xE1 | PROD_BATTERY_MODEL | 20B | PC→WB7720→NU17112 | Flash `ProductInfo_t.battery_model` |
| 0xE2~0xF5 | PROD_PROD_DATE | 20B | PC→WB7720→NU17112 | Flash `ProductInfo_t.battery_prod_date` |
